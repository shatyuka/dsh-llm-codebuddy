...
```bash
pnpm dsh plugin --profile web add @shatyuka/dsh-llm-codebuddy
```
...
```bash
pnpm dsh plugin --profile web dsh-codebuddy-login              # sign in
pnpm dsh plugin --profile web dsh-codebuddy-login --status     # who is signed in, plus the model list
pnpm dsh plugin --profile web dsh-codebuddy-login --logout     # remove the stored credential
```
